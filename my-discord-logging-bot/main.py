...
from flask_socketio import SocketIO, emit, join_room, leave_room, close_room # Added SocketIO specific imports
...
# This data is now part of your main.py. Ensure passwords are dummy values.
WHITELIST_DATA = [
  {
  ...
  {
    "name": "Curtis", "username": "Zapperix", "password": "DEMO_PASSWORD_Zapperix", # <<< Corrected 'username' key
...
# It's important to set this up early, before Discord bot for socketio to be managed correctly.
# Eventlet is required by Flask-SocketIO.
eventlet.monkey_patch() # Patch standard library to be non-blocking.
from flask import Flask, request, jsonify, send_from_directory # re-import if patched modules are required.
...
# We associate logged-in user_ids with their Socket.IO SIDs (session IDs)
# A user could have multiple tabs open, each with a different SID.
user_sid_map = collections.defaultdict(list) # {user_id: [sid1, sid2, ...]}
sid_user_map = {} # {sid: user_id}
RESET_CODE_SERVER_SECRET = "Reset.2579" # Reset code moved to server-side!

# Init SocketIO, tying it to the Flask app
# `async_mode='eventlet'` makes it compatible with eventlet patching
# which allows it to run non-blocking alongside asyncio tasks
socketio = SocketIO(web_app, async_mode='eventlet', cors_allowed_origins="*") # Allow all origins for dev/deploy ease
...
# Endpoint for client-side login requests (Server-Side Authentication & Logging)
...
def api_handle_login():
    ...
    current_attempts = getattr(web_app, 'login_attempts', 3) # Initial/server-stored attempts
    setattr(web_app, 'login_attempts', current_attempts - 1)

    logger.info(f"[WEB_API] Login attempt by '{username}' (ID: {userID}) from IP: {request.remote_addr} (Attempts Left: {current_attempts})")

    # Frontend webhooks for login events will still be sent from script.js,
    # as per user's previous explicit instruction to keep webhooks client-side.
    # The server side merely provides the authentication validation here.
    ...
    if user_found:
        setattr(web_app, 'login_attempts', 3) # Reset attempts on successful login
        logger.info(f"[WEB_API] User '{username}' (ID: {userID}) successfully logged in.")
        return jsonify({"status": "success", "message": "Login successful.", "attempts_left": 3}), 200
    else:
        logger.warning(f"[WEB_API] Login failed for '{username}' (ID: {userID}).")
        if web_app.login_attempts <= 0:
            logger.critical(f"[WEB_API] Login attempts exhausted for IP {request.remote_addr}. System locked.")
            return jsonify({"status": "error", "message": "Invalid credentials. System locked.", "attempts_left": 0}), 401
        else:
            return jsonify({"status": "error", "message": "Invalid credentials.", "attempts_left": web_app.login_attempts}), 401


# Endpoint for client-side reset requests (Server-Side Reset Logic & Logging)
@web_app.route('/api/reset_attempts', methods=['POST'])
def api_handle_reset_attempts():
    data = request.json
    reset_code = data.get('reset_code')
    client_info = data.get('client_info', {}) # Capture client info if passed

    logger.info(f"[WEB_API] Reset attempt from IP: {request.remote_addr} with code: {reset_code}")

    if reset_code == RESET_CODE_SERVER_SECRET:
        setattr(web_app, 'login_attempts', 3) # Reset attempts
        logger.info(f"[WEB_API] Login attempts reset by code from IP: {request.remote_addr}")
        # Frontend webhooks for reset events will still be sent from script.js.
        return jsonify({"status": "success", "message": "Login attempts reset successfully!", "new_attempts_count": 3}), 200
    else:
        logger.warning(f"[WEB_API] Incorrect reset code from IP: {request.remote_addr}")
        return jsonify({"status": "error", "message": "Incorrect reset code. Access denied."}), 403


# --- Socket.IO Event Handlers ---
@socketio.on('connect')
def handle_connect():
    # 'request.sid' is the Socket.IO session ID
    user_id = request.args.get('user_id') # Can be passed in the handshake
    username = request.args.get('username')

    if user_id:
        join_room(user_id) # Each logged-in user gets their own Socket.IO room
        user_sid_map[user_id].append(request.sid)
        sid_user_map[request.sid] = user_id
        logger.info(f"Socket.IO client connected. SID: {request.sid}, UserID: {user_id}")
    else:
        logger.info(f"Socket.IO client connected without user_id. SID: {request.sid}")
...
def handle_disconnect():
    ...
    if user_id:
        if sid in user_sid_map[user_id]:
            user_sid_map[user_id].remove(sid)
        if not user_sid_map[user_id]: # If no more SIDs for this user
...
def handle_user_login_from_client(data):
    ...
    username = data.get('username')
    logger.info(f"Client reported login via Socket.IO. UserID: {user_id}, Username: {username}")
    # You might emit a broadcast here if needed, or update server-side 'online' status


# --- Discord Bot's Backend-facing API (called by Discord bot commands) ---
...
def api_get_user_info_for_bot(user_id):
    ...
    if user_info:
        ...
        safe_user_info["is_online"] = True if user_id in user_sid_map else False # Real-time online check via Socket.IO
...
def api_get_online_users_for_bot():
    """Returns a list of currently online users via Socket.IO connection status."""
    online_users_list = []
    for user_id, sids in user_sid_map.items():
        if sids: # If user_id has active SIDs
...
# (All other /api/users/<user_id>/stats, /api/users/<user_id>/session_time etc. remain the same)
# ... Mock for user stats, session time, notes, device info ...

@web_app.route('/api/users/<user_id>/stats', methods=['GET'])
def api_get_user_stats_for_bot(user_id):
    user_info = next((user for user in WHITELIST_DATA if str(user["userID"]) == user_id or user["username"] == user_id), None)
    if user_info:
        stats_data = {"username": user_info["username"], "total_clicks": 100 + (int(user_id) % 50 if user_id.isdigit() else 0), "session_count": 5 + (int(user_id) % 3 if user_id.isdigit() else 0), "avg_session_time": f"{1 + (int(user_id) % 2 if user_id.isdigit() else 0)}h {(10 + (int(user_id) % 30 if user_id.isdigit() else 0))}m", "error_count": (int(user_id) % 5 if user_id.isdigit() else 0), "last_activity": "2025-08-25 10:30:00Z" }
...
# --- API Endpoints for Bot-Triggered Actions (Using Socket.IO) ---
...
def api_bot_logout_user():
    ...
    # --- Action: Emit Socket.IO event to client(s) ---
    if user_id in user_sid_map and user_sid_map[user_id]:
        socketio.emit('server_command', {'command': 'logout', 'message': 'You have been logged out by administrator.'}, room=user_id)
        # Force disconnect all SIDs for this user for immediate effect (optional, depends on client handling)
        for sid in list(user_sid_map[user_id]): # Iterate a copy
            socketio.emit('disconnect', {}, room=sid, namespace='/') # Direct disconnect. client must handle
            socketio.disconnect(sid, namespace='/')
        logger.info(f"Issued Socket.IO logout command and disconnected clients for UserID: {user_id}")
    else:
        logger.warning(f"No active Socket.IO connection for UserID: {user_id} to logout.")
...
def api_bot_panic_user():
    ...
    if user_id in user_sid_map and user_sid_map[user_id]:
        socketio.emit('server_command', {'command': 'panic', 'url': url}, room=user_id)
        logger.info(f"Issued Socket.IO panic command to UserID: {user_id} to URL: {url}")
    else:
        logger.warning(f"No active Socket.IO connection for UserID: {user_id} to panic.")
...
def api_bot_zoom_user():
    data = request.json
    user_id = data.get('user_id')
    level = data.get('level') # This comes as int (e.g. 100, 125)
    logger.info(f"[WEB_API] Bot requested zoom level {level} for user {user_id}.")
    if user_id in user_sid_map and user_sid_map[user_id]:
        socketio.emit('server_command', {'command': 'zoom', 'level': level}, room=user_id)
        logger.info(f"Issued Socket.IO zoom command to UserID: {user_id}, Level: {level}%")
    else:
        logger.warning(f"No active Socket.IO connection for UserID: {user_id} to change zoom.")
...
def api_bot_screenshot_user():
    ...
    if user_id in user_sid_map and user_sid_map[user_id]:
        # Frontend JS needs to implement receiving 'screenshot' command, taking screenshot,
        # uploading it back to an /api/screenshot-upload endpoint, then server can return its URL
        socketio.emit('server_command', {'command': 'screenshot_request', 'message': 'Please capture your screen.'}, room=user_id)
        logger.info(f"Issued Socket.IO screenshot request to UserID: {user_id}")
        return jsonify({"status": "success", "message": f"Screenshot request for {user_id} sent. Awaiting upload from client.", "data": {"image_url": mock_image_url}}), 200 # Returns mock URL for now
    else:
        logger.warning(f"No active Socket.IO connection for UserID: {user_id} to request screenshot.")
        return jsonify({"status": "success", "message": f"Screenshot request for {user_id} failed: client offline.", "data": {"image_url": None}}), 200

# (Add all other `/api/user/*` endpoints that your Discord bot commands are calling from the Flask API, e.g., clear updates, set clicks, set theme, etc. They will mostly be like the logout/panic endpoints above, emitting to a Socket.IO room.)

# --- Mock Implementations for other bot-triggered actions ---
...
def api_bot_clear_updates():
    ...
    socketio.emit('server_command', {'command': 'clear_updates'}, room=user_id); return jsonify({"status": "success", "message": f"Clear updates for {user_id} sent."}), 200
...
def api_bot_clear_notifications():
    ...
    socketio.emit('server_command', {'command': 'clear_notifications'}, room=user_id); return jsonify({"status": "success", "message": f"Clear notifications for {user_id} sent."}), 200
...
def api_bot_clear_activity():
    ...
    socketio.emit('server_command', {'command': 'clear_activity'}, room=user_id); return jsonify({"status": "success", "message": f"Clear activity for {user_id} sent."}), 200
...
def api_bot_clear_errors():
    ...
    socketio.emit('server_command', {'command': 'clear_errors'}, room=user_id); return jsonify({"status": "success", "message": f"Clear errors for {user_id} sent."}), 200
...
def api_bot_clear_login_history():
    ...
    socketio.emit('server_command', {'command': 'clear_login_history'}, room=user_id); return jsonify({"status": "success", "message": f"Clear login history for {user_id} sent."}), 200
...
def api_bot_clear_all_data():
    ...
    socketio.emit('server_command', {'command': 'clear_all_data', 'warning': 'All data wiped'}, room=user_id); return jsonify({"status": "success", "message": f"Clear all data for {user_id} sent."}), 200
@web_app.route('/api/user/set_clicks', methods=['POST'])
def api_bot_set_clicks():
    data = request.json; user_id = data.get('user_id'); count = data.get('count')
    logger.info(f"[WEB_API] Bot requested to set clicks for user {user_id} to {count}.")
    socketio.emit('server_command', {'command': 'set_clicks', 'count': count}, room=user_id); return jsonify({"status": "success", "message": f"Set clicks for {user_id} to {count} sent."}), 200
...
def api_bot_clear_clicks():
    ...
    socketio.emit('server_command', {'command': 'clear_clicks'}, room=user_id); return jsonify({"status": "success", "message": f"Clear clicks for {user_id} sent."}), 200
@web_app.route('/api/user/set_announcement', methods=['POST'])
def api_bot_set_announcement():
    data = request.json; user_id = data.get('user_id'); message = data.get('message')
    logger.info(f"[WEB_API] Bot requested to set announcement for user {user_id}: {message}.")
    socketio.emit('server_command', {'command': 'set_announcement', 'message': message}, room=user_id); return jsonify({"status": "success", "message": f"Set announcement for {user_id} sent."}), 200
...
def api_bot_restart_page():
    ...
    socketio.emit('server_command', {'command': 'restart_page'}, room=user_id); return jsonify({"status": "success", "message": f"Restart page for {user_id} sent."}), 200
@web_app.route('/api/user/set_theme', methods=['POST'])
def api_bot_set_theme():
    data = request.json; user_id = data.get('user_id'); theme_name = data.get('theme_name')
    logger.info(f"[WEB_API] Bot requested to set theme for user {user_id} to {theme_name}.")
    socketio.emit('server_command', {'command': 'set_theme', 'theme_name': theme_name}, room=user_id); return jsonify({"status": "success", "message": f"Set theme for {user_id} sent."}), 200
@web_app.route('/api/user/set_dashboard_color', methods=['POST'])
def api_bot_set_dashboard_color():
    data = request.json; user_id = data.get('user_id'); color = data.get('color')
    logger.info(f"[WEB_API] Bot requested to set dashboard color for user {user_id} to {color}.")
    socketio.emit('server_command', {'command': 'set_dashboard_color', 'color': color}, room=user_id); return jsonify({"status": "success", "message": f"Set dashboard color for {user_id} sent."}), 200
@web_app.route('/api/user/set_event', methods=['POST'])
def api_bot_set_event():
    data = request.json; user_id = data.get('user_id'); event_name = data.get('event_name'); message = data.get('message')
    logger.info(f"[WEB_API] Bot requested to set event for user {user_id}: {event_name} - {message}.")
    socketio.emit('server_command', {'command': 'set_event', 'event_name': event_name, 'message': message}, room=user_id); return jsonify({"status": "success", "message": f"Set event for {user_id} sent."}), 200
@web_app.route('/api/user/control_section', methods=['POST'])
def api_bot_control_section():
    data = request.json; user_id = data.get('user_id'); action = data.get('action'); section = data.get('section')
    logger.info(f"[WEB_API] Bot requested to {action} section {section} for user {user_id}.")
    socketio.emit('server_command', {'command': 'control_section', 'action': action, 'section': section}, room=user_id); return jsonify({"status": "success", "message": f"Control section for {user_id} ({action} {section}) sent."}), 200

# (All other command endpoints need to be added similarly with SocketIO.emit calls)


# --- Discord Bot Core ---
# IMPORTANT: help_command=None disables the default Discord.py help command.
# Otherwise, your custom @bot.command(name='help') would clash.
...
# --- Discord Bot's internal Website Interaction functions (These are used BY DISCORD COMMANDS) ---
...
# These can be called by the Flask app or bot, if some internal logic needs Discord bot to announce.
...
async def bot_user_disconnected_announcement(user_id: str, username: str = "Unknown User"):
    ...
    else: logger.warning(f"Configured DISCONNECTED_CHANNEL_ID ({DISCONNECTED_CHANNEL_ID}) not found or bot lacks permissions. Cannot announce user '{username}' disconnected.")
...
# ... (rest of help message as before) ...
"""
...
@is_admin()
async def screenshot_user(ctx, user_id: str): await ctx.send(f"Requesting screenshot for user `{user_id}`... This may take a moment."); try: response = await website_send_command("/api/user/screenshot", user_id=user_id); if response.get("status") == "success" and response.get("data", {}).get("image_url"): image_url = response["data"]["image_url"]; await ctx.send(f"Screenshot for `{user_id}`:"); embed = discord.Embed(title=f"Screenshot for {user_id}", color=0x3498db); embed.set_image(url=image_url); await ctx.send(embed=embed) else: await ctx.send(f"Screenshot request for `{user_id}` failed or returned no image URL. API Message: {response.get('message', 'No message.')}") except Exception as e: logger.error(f"Error in screenshot_user command for user '{user_id}' by {ctx.author}: {e}", exc_info=True, extra={'command_name': 'Screenshot', 'user_id': user_id, 'user_name': str(ctx.author)}); await ctx.send(f"An error occurred while requesting screenshot for user `{user_id}`. Please try again later.")
...
def start_discord_bot_in_thread(discord_bot_instance, token):
    ...
    try:
        ...
        # Don't let discord.py setup its own logger handler, our custom is already there.
        bot_loop.run_until_complete(discord_bot_instance.start(token)) # Use bot.start with provided token
    except discord.errors.LoginFailure as e:
        logger.critical(f"Discord Bot Login Failed in thread: Invalid Token. {e}", exc_info=True)
        # Cannot use async_run_coroutine_threadsafe directly from non-main thread in case main loop not started.
        # Fallback to main loop after some time, or simpler: os._exit.
        # However, Flask is in the main thread now, so the bot_loop has access.
...
if __name__ == '__main__':
    ...
    # Pass bot instance and token to the thread target function
    ...
    # Use socketio.run instead of web_app.run when Flask-SocketIO is active.
    # It also handles web_app (Flask routes) implicitly.
    try:
        logger.info(f"Starting Flask-SocketIO web app in MAIN thread on 0.0.0.0:{WEB_SERVER_PORT} for Render Web Service...")
        # `web_app` is your Flask application instance.
        socketio.run(web_app, host='0.0.0.0', port=WEB_SERVER_PORT, debug=False, allow_unsafe_werkzeug=True, log_output=True)
        # allow_unsafe_werkzeug=True needed on older versions or if certain werkzeug settings.
    except Exception as e:
