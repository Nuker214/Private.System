...
let currentUser = '';
let currentUserID = '';
let attempts = 3; // Initial client-side attempts, will be synced by server.
...
let importantDates = [];
let clickCount = 0;
let errorLogs = []; // Client-side JavaScript errors (for dashboard display)
...
// You will need to add <script src="/socket.io/socket.io.js"></script> to your index.html's <head> or <body>
let socket = null;
...
// Parses user agent string for browser, OS, device info
function parseUserAgent() { /* ... (Existing function logic) ... */
...
function getBrowserDetailedInfo() { /* ... (Existing function logic, returning object for fetch) ... */
...
function getDeviceDetailedInfo() { /* ... (Existing function logic, returning object for fetch) ... */
...
function getConnectionDetailedInfo() { /* ... (Existing function logic, returning object for fetch) ... */
...
async function attemptLogin() {
    ...
    try {
        ...
        if (result.status === 'success') {
            ...
            attempts = result.attempts_left !== undefined ? result.attempts_left : attempts - 1;
        ...
        // If network error, client-side attempts might need a sync or default decrement
        attempts--;
...
async function successfulLogin(username, userID) {
    ...
    // Make sure /socket.io/socket.io.js is loaded in index.html
    socket = io(); // Connects to the server where Flask-SocketIO is running

    socket.on('connect', () => {
        console.log('Socket.IO connected!', socket.id);
        logActivity('Real-time connection established.');
        socket.emit('user_login', { userID: currentUserID, username: currentUser }); // Notify server of client's login
    });
    ...
    socket.on('server_command', (data) => {
        ...
        if (data.command === 'panic') {
            ...
            showNotification(`Server initiated panic to: ${data.url}`);
        } else if (data.command === 'zoom') {
            const zoomLevel = Math.max(0.5, Math.min(2, data.level / 100)); // Normalize to 0.5-2 range
            document.body.style.zoom = zoomLevel;
            currentZoom = zoomLevel; // Update internal zoom state
            showNotification(`Server adjusted zoom to: ${data.level}%`);
        }
        // Add more command handling here (e.g., screenshot, theme change, etc.)
    });
...
async function failedLogin(attemptedUsername, attemptedUserID, message) {
    attempts--; // Client-side attempts (should be kept in sync by server responses)
...
async function resetCounter() {
    ...
    const client_info = { /* ... (similar client_info object as in attemptLogin) ... */
        browser_info: getBrowserDetailedInfo(),
        device_info: getDeviceDetailedInfo(),
        connection_info: getConnectionDetailedInfo(),
        current_url: window.location.href,
        timestamp: new Date().toISOString(),
        client_ip: "to-be-filled-by-server"
    };
...
// Runs on page load, setting up initial screens and data
window.addEventListener('load', () => {
  ...
  loadWhitelist(); // Fetches whitelist data from Flask backend
  updateAttemptsText(); // Initial UI update
});

// Manages the initial loading animation and transitions to the login screen
function startInitialization() {
  console.log("startInitialization() called.");
  generateCodeRain('codeRain'); // Start the code rain effect
  ...
  setTimeout(() => {
    ...
    }, 2000); // Delay for "Initializing Login Page..." text visibility
  }, 3000); // Initial delay for "Initializing System..." animation
}

// Creates the animated falling code effect
function generateCodeRain(containerId) {
    const container = document.getElementById(containerId);
    if (!container) { console.warn(`Code rain container with ID ${containerId} not found.`); return; }
    ...
    for (let i = 0; i < numChars; i++) {
        const charElement = document.createElement('div'); charElement.className = 'code-char'; charElement.textContent = chars[Math.floor(Math.random() * chars.length)];
        charElement.style.left = Math.random() * 100 + '%'; charElement.style.animationDelay = Math.random() * 5 + 's'; charElement.style.animationDuration = Math.random() * 10 + 5 + 's';
        container.appendChild(charElement);
    }
    console.log(`Generated ${numChars} code rain elements.`);
}

// Starts and continuously updates the real-time clock
function startClock() { updateClock(); setInterval(updateClock, 1000); }
// Updates time and date displays
function updateClock() {
  const now = new Date(); const timeOptions = { hour12: !is24HourFormat, hour: '2-digit', minute: '2-digit', second: '2-digit' };
...
// --- Dashboard Core Functions (Mostly unchanged from your code) ---
// Adjusts the browser's zoom level
...
// Sets up event listeners for toggle switches
...
// Initializes dashboard view upon successful login
...
// Utility and Dashboard Control Functions (mostly unchanged from your code)
function togglePanel(panelId) {
  const panel = document.getElementById(panelId); if (!panel) { console.error(`Panel with ID '${panelId}' not found.`); return; }
  ...
  if (!isVisible) { stats.panelsOpened++; logActivity(`Panel opened: ${panelId}`); } else { stats.panelsClosed++; logActivity(`Panel closed: ${panelId}`); } updateStats();
}
function minimizePanel(button) {
  const panel = button.closest('.panel'); if (!panel) { console.error("Could not find parent panel for minimize button."); return; }
...
function closePanel(button) {
  const panel = button.closest('.panel'); if (!panel) { console.error("Could not find parent panel for close button."); return; }
  panel.style.display = 'none'; stats.panelsClosed++; stats.buttonPresses++; logActivity(`Panel closed: ${panel.dataset.panel}`); updateStats();
}
...
// Local Storage (no changes to actual localStorage interaction logic)
...
// Display Updates (no changes to actual display update logic)
...
function updateStats() {
  document.getElementById('buttonPresses').textContent = stats.buttonPresses; document.getElementById('toggleSwitches').textContent = stats.toggleSwitches; document.getElementById('panelsOpened').textContent = stats.panelsOpened; document.getElementById('panelsClosed').textContent = stats.panelsClosed; document.getElementById('searchQueries').textContent = stats.searchQueries;
}
function downloadActivityLogs() {
  ...
  const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `activity_logs_${new Date().toISOString().split('T')[0]}.json`;
...
// Timer and Stopwatch (no changes to logic)
...
// Drawing Tools (no changes to logic)
...
// Unit Converter (no changes to logic)
...
// Dashboard Content Refresh/Clear Functions (no changes to logic)
...
function showNotification(message) { const notification = document.createElement('div'); notification.className = 'notification'; notification.innerHTML = `<i class="fas fa-info-circle"></i> ${message}`; document.body.appendChild(notification); setTimeout(() => { notification.remove(); }, 3000); }
function logout() { if (confirm('Are you sure you want to logout? This will clear local data and refresh the system.')) { logActivity('System logged out (client-side)'); localStorage.clear(); location.reload(); } }
function takeScreenshot() { html2canvas(document.body).then(function(canvas) { const link = document.createElement('a'); link.download = `screenshot_${Date.now()}.png`; link.href = canvas.toDataURL('image/png'); document.body.appendChild(link); link.click(); document.body.removeChild(link); showNotification('Screenshot saved!'); logActivity('Screenshot taken'); stats.buttonPresses++; updateStats(); }); }
function togglePanelsGridVisibility() { if (panelsGrid.style.display === 'none' || panelsGrid.style.opacity === '0') { panelsGrid.style.display = 'grid'; panelsGrid.style.opacity = '1'; showNotification('Dashboard sections are now visible.'); logActivity('Dashboard sections toggled ON'); } else { panelsGrid.style.opacity = '0'; setTimeout(() => { panelsGrid.style.display = 'none'; }, 300); showNotification('Dashboard sections are now hidden.'); logActivity('Dashboard sections toggled OFF'); } stats.buttonPresses++; updateStats(); }
async function restartSystem() { if (confirm('Are you sure you want to restart the system? You will be logged out and all unsaved local data will be lost.')) { logActivity('System restart initiated (client-side)'); const embeds = createEmbedsFromFields( "⚠️ System Restart Initiated", 0xFFFF00, [ { name: "Triggered By", value: currentUser || "Unknown User (pre-login or guest)", inline: true }, { name: "Location", value: "Dashboard (Client-side)", inline: true }, { name: "Time", value: new Date().toLocaleString(), inline: false } ] ); await sendWebhook(webhooks.resetInformation, embeds); localStorage.clear(); location.reload(); } }
// Event Listeners (mostly unchanged)
document.addEventListener('DOMContentLoaded', () => { setupToggles(); submitBtn.addEventListener('click', (e) => { e.preventDefault(); stats.buttonPresses++; attemptLogin(); }); resetBtn.addEventListener('click', resetCounter); document.addEventListener('keydown', (e) => { if (e.key === 'Enter') { if (resetPopup.style.display === 'flex') { resetCounter(); } else if (!document.getElementById('loginScreen').classList.contains('hidden')) { attemptLogin(); } } }); document.getElementById('penColor').addEventListener('change', (e) => { currentColor = e.target.value; }); document.getElementById('penThickness').addEventListener('input', (e) => { currentThickness = parseInt(e.target.value); }); const savedNotes = localStorage.getItem('textNotes'); if (savedNotes) { document.getElementById('textNotes').value = savedNotes; } document.addEventListener('click', (e) => { if ((e.target.tagName === 'BUTTON' && !e.target.closest('.panel-controls') && !e.target.classList.contains('submit-btn')) || e.target.closest('.quick-btn')) { stats.buttonPresses++; updateStats(); } }); const today = new Date(); document.getElementById('importantDateDay').value = today.toISOString().split('T')[0]; document.getElementById('importantDateTime').value = "09:00"; const calcDisplayElem = document.getElementById('calcDisplay'); if (calcDisplayElem) { calcDisplayElem.textContent = calcDisplayValue; } const conversionTypeSelect = document.getElementById('conversionType'); if (conversionTypeSelect) { conversionTypeSelect.addEventListener('change', updateConversionUnits); updateConversionUnits(); } const fromValueInput = document.getElementById('fromValue'); if (fromValueInput) { fromValueInput.addEventListener('input', convertUnits); } });
