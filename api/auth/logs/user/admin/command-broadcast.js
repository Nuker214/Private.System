    ...
    await cors(corsOptions)(req, res, async () => {
        ...
        return await authenticateToken(authorizeAdmin(async (req, res) => {
            ...
            try {
                ...
                await pusher.trigger('public-broadcast', 'admin-command', {
                    command,
                    payload,
                    sender: req.user.username
                });
                res.json({ msg: `Command '${command}' broadcast to all online users` });
            } catch (err) {
                console.error('Pusher error:', err.message);
                res.status(500).send('Server Error broadcasting command');
            }
        }))(req, res);
    });
};
